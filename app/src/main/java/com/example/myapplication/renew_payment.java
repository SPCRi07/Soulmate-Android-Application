...
public class renew_payment extends Activity implements PaymentResultListener {
    ...
    public void onPaymentError(int i, String s) {
        ...
        Intent i1=new Intent(renew_payment.this,BottomNavigationActivity.class);
        startActivity(i1);
    }
    private String time()
        {
                ...
                try {
                    if(response.getString("res").equals("ok")) {
                            ...
                            for(int i=0;i<ja.length();i++) {
                               ...
                               uservalues.fullname=jo.getString("name");
                            }
                     }
                    else
                    {
                        Toast.makeText(getApplicationContext(),"FAILED to retrieve profile",Toast.LENGTH_SHORT).show();
                    }
    ...
    private void addtosubscription(){
        ...
        client.post(Constants.AddtoSubscription,rp,new JsonHttpResponseHandler(){
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    if(response.getString("res").equals("ok")) {
                        Toast.makeText(getApplicationContext(),"Success to retrieve profile",Toast.LENGTH_SHORT).show();
                    }
                    else {
                        Toast.makeText(getApplicationContext(),"Failed to add into Subscription",Toast.LENGTH_SHORT).show();
                        }
                } catch (JSONException ex) {
                    ex.printStackTrace();
                }
